...
- **shellEnv.ts** — `fixProcessPath()` enriches `process.env.PATH` at startup by spawning a login shell (`zsh -ilc`) and adding well-known CLI directories (`~/.local/bin`, `/usr/local/bin`, `/opt/homebrew/bin`). Fixes Electron's restricted PATH when launched from Dock/Finder. Called in `main/index.ts` before IPC setup.
...
- **paths.ts** — `getConfigDir()` returns platform-appropriate config directory (macOS/Linux: ~/.lumi, Windows: %APPDATA%/lumi). In dev mode (`NODE_ENV=development`), appends `-dev` suffix (`~/.lumi-dev`, `%APPDATA%/lumi-dev`) to isolate dev data from production. Legacy migration (.pulpo, .ai-orchestrator) only runs in production. Also exports `getTempDir()` for ephemeral files (prod: `os.tmpdir()/lumi`, dev: `os.tmpdir()/lumi-dev`).
- **systemChecks.ts** — `getPlatformChecks()` returns platform-specific health checks (macOS: spawn-helper, Windows: ConPTY version). Spawn-helper resolution checks both packaged (`Contents/Resources/app.asar.unpacked/node_modules/node-pty`) and dev (`node_modules/node-pty`) locations and reports status only (no auto-fix).
