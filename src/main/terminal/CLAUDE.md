...
- **types.ts** — `ManagedTerminal`, `SpawnResult` (re-exported from shared), `ITerminalNotifier`, `ICodenameTracker`, `IPtyDataInspector` interfaces
- **PtyRawLogger.ts** — Raw PTY data file logger; writes complete PTY output (visible text + escape sequences) to `~/.ai-orchestrator/pty-logs/` using async WriteStream; each terminal gets a separate log file with chunk headers (timestamp, sequence, byte count); non-printable chars hex-escaped (`\xHH`); zero-cost when disabled
...
- TerminalManager uses dependency injection: `ITerminalNotifier`, `ICodenameTracker`, and optional `IPtyDataInspector` interfaces (DIP)
- TerminalManager calls `inspector.onTerminalExit(id)` in the `onExit` handler to clean up log streams
...
- Terminal exit handler cleans up from both the Map, notifier, and inspector
