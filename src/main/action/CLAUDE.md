...
## Architecture
- **ActionStore** — loads/watches YAML from `~/.lumi/actions` (user) and `<repo>/.lumi/actions` (project). Project scope overrides user scope by action `id`.
...
- Backups stored in `~/.lumi/actions/.history/<action-id>/` as `<ISO-timestamp>.yaml`.
